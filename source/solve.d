...
struct FiniteVolume
{
	this(immutable Mesh _mesh, immutable IConfig _config)
	{
		parent = SSolver(_mesh);
		config = _config.to!(immutable FEMConfig);
		flux = new Vec[mesh.edges.length];
		edgeQ = new Vec[mesh.edges.length*2];
	}

	@nogc private void invert(ref Matrix!(2, 2) inv, ref Matrix!(2, 2) mat)
	{
		inv[0] = mat[3];
		inv[1] = -mat[1];
		inv[2] = -mat[2];
	...
	@nogc private void LP(uint m)(ref Matrix!(m, 2) A, ref Vector!m b, ref Vector!2 c, ref Vector!2 xk)
	{
		uint[2] Wk = [0, 1];
	...
	@nogc void ufvmSetup(ref Mesh mesh, ref Config config, double[] lastRho, double[] lastU, double[] lastV, double[] lastE, ref double t, ref double dt, string saveFile)
	{
	...
	@nogc private void updateBoundaryEdge(size_t dims)(ref Mesh mesh, Vector!dims[] q, uint i, Config config)
	{
		if(config.order == 1)
		{
	...
	@nogc void ufvmSolver(alias F, size_t dims)(ref Vector!dims[] R, ref Vector!dims[] q, ref Mesh mesh, Config config, ref double newDt, ref double Rmax, bool limit, bool dtUpdate)
	{
